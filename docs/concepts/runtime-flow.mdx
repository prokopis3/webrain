...
- **`webrain-mcp`** — the MCP server: stdio + HTTP transports, the JSON-RPC
  dispatcher, and the 16 consolidated tool schemas + dispatch table.
...
17-tool surface (`observe`/`interact`/`extract`/`crawl`/`session`/…) is folded
to these executors up front by `tools::map_surface()`.

### 3. Tool registration — `webrain-mcp/src/tools.rs`

- **`list_tools()`** — the **16 consolidated `webrain_*` schemas** the LLM
