...
| 9 | **Rate limiter + sticky proxy** — exp-backoff on 429/503 (`base/max_delay/max_retries`); pin one IP to a deep-crawl session (`get_proxy_for_session(ttl)`) | Survives rate-limit walls; deep crawls keep a consistent exit IP. A single per-call `--proxy`/`proxy` exists today (SERP) — the rotation half is below. | Medium (~120 ln) |
| 9b | **Proxy rotation pool + challenge-triggered rotate** (open-serp parity) — `WEBRAIN_PROXIES` list, round-robin `next()`, `mark_dead()` on wall/429, auto-retry next proxy; per-proxy cookie lanes (`DropCookiesOnChallenge`) | The missing half of the SERP `--proxy` feature: rotates clean IPs on flagged/walled engines instead of one manual proxy. | Medium (~120 ln) |
