...
description: Which webrain MCP tool should your AI agent use? Decision table mapping every task — navigate, observe, interact, extract, crawl, batch, PDF, video, sessions — to the 16 webrain tools.
---

# Agent Decision Guide

webrain exposes **17 intent-based MCP tools**. This page is a decision table:
...
| Typed SERP JSON (position/title/url/domain/snippet) | `webrain_serp` | `engine=…` |
...
| Run JS inside a specific cross-origin iframe (isolated world, reCAPTCHA/Turnstile frames) | `webrain_eval_in_frame` | `url_contains` + `js` |
...
**Search the web / structured results** → `webrain_search` (navigate to the
results page) or `webrain_serp` (typed JSON: position/title/url/domain/snippet).
...
| "Get the top search results as JSON" | `webrain_serp` | `engine=duckduckgo` |
