...
Webrain is an **MCP server** — register it in your client and its **16
intent-based tools** appear automatically (63 legacy one-action names still
dispatch for backward compatibility). Two transports:
...
Full per-client walkthroughs: [Claude](/mcp/claude) · [Cursor](/mcp/cursor) ·
[GitHub Copilot](/mcp/copilot) · [MCP Server](/mcp/server).
...
`webrain_extract` (mode=schema) and returns structured data. No selectors on
your part.
