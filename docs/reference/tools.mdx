...
Structured search results as **typed JSON** (`position`/`title`/`url`/`domain`/`snippet`) — a SERP API any LLM can call. `duckduckgo` (default) / `bing` / `google` are plain HTML served over the pooled no-browser HTTP agent; `brave` is a JS SPA rendered in the connected CDP engine (Chrome/obscura/lightpanda); `auto` fetches all HTTP engines concurrently and merges + dedupes.
