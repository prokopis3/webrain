...
**serpapi.com fallback:** set the standard `SERPAPI_API_KEY` env var and google falls back to the paid serpapi.com API (`/search.json`, engine=google) whenever the browser path walls — a clean, no-browser Google provider for `--engine google`, `auto`, and fallback chains. Unset key / quota-exhausted / 4xx degrades to the normal fallback with no error.

```json
{ "q": "product reviews", "engine": "google", "limit": 20, "proxy": "http://user:pass@host:port" }
```

Also on the CLI: `webrain serp "query" [--engine …] [--limit N] [--page N] [--safe] [--region R] [--no-fallback] [--json] [--headless] [--proxy URL]`.
</Accordion>
