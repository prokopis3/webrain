...
| Interact (`click`, `type`, `press`, `scroll`, `nav`, `tab`, `dismiss_overlays`) | `webrain-core/src/backends/cdp.rs` | ✅ CDP | index/selector based; Material widgets need real Chrome |
| Extract (`autoschema`, `extract_json`, `extract_regex`, `table`, `jsonld`, `bm25`, `clean`) | `webrain-core/src/engines.rs` + in-page JS | ✅ CDP (in-page) | zero-LLM, no `get_html` |
...
| PDF & Media (`pdf*`, `download`, `media`, `get_images`) | `webrain-core` pdf modules + `engines.rs` | 🔶 mixed | `download` (http + yt-dlp) and `pdf_extract` need no browser |
...
`webrain_download`, `webrain_pdf_extract`) are intercepted in
